...
def train(discriminator, classifier, dataloader_src, dataloader_tgt, train_epochs, domain_label, loss_weight):
    ...
    """
    print('parameters:', train_epochs, domain_label, loss_weight)
    ...
    for epoch in range(train_epochs[0]):
        ...
        for step, ((imgs_src, labels_src), (imgs_tgt, labels_tgt)) in data_zip:
            ...
            tgt_imgs = feature_fooling_tgt.reshape(feature_fooling_tgt.shape[0], 3, params.imgs_size, params.imgs_size)
            ...
            crf_imgs = feature_fooling_src.reshape(feature_fooling_src.shape[0], 3, params.imgs_size, params.imgs_size)
            ...
            for i_c in range(train_epochs[3]):
                ...
                # optimizer_c.step()
                feature_fooling_src = Variable(feature_fooling_src, requires_grad=True)
            src_imgs = feature_fooling_src.reshape(feature_fooling_src.shape[0], 3, params.imgs_size, params.imgs_size)
        ...
        if (epoch+1) % 50 == 0:
            acc_src = 0
            for (imgs_src, labels_src) in dataloader_tgt:
        ...
        if epoch > 1000000:
            # Save samples
            generate_imgs = torch.cat((src_imgs[50:70], tgt_imgs[50:70], crf_imgs[50:70]), 0)
            save_image(generate_imgs, '../generate_samples/{}.png'.format(epoch), nrow=10, normalize=True)
...
if __name__ == '__main__':
    ...
    train_epochs = [[150, 20, 20, 20], [150, 10, 10, 10], [150, 20, 10, 10], [150, 50, 20, 20], [150, 5, 5, 5],
                    [150, 10, 20, 20], [150, 20, 20, 10]]
    # domain label
    domain_label = [[1.0, 0.0, 1.0, 0.0], [1, 0, 0.8, 0.2], [1, 0.0, 0.8, 0.4], [0.9, 0.0, 0.8, 0.4], [1, 0.0, 0.4, 0.8],
                    [1, 0.0, 0.5, 0.5], [1.0, 0.0, 0.1, 0.8]]
    # 各种loss的权重  loss_c_src + dloss + dloss_f + loss_c_f_src + dis + entropy
    loss_weight = [[1, 0.5, 0.5, 1.0, 1.0, 0.1], [1, 0.5, 0.5, 1.0, 1, 0.2], [1, 0.5, 0.5, 1.0, 1, 0.01], [1, 0.5, 0.5, 1.0, 1.0, 5],
                   [1, 1, 1, 1.0, 1.0, 0.1], [1, 5, 5, 1.0, 1.0, 0.1], [0.1, 0.5, 0.5, 1.0, 1.0, 0.1]]

    for i in range(len(train_epochs)):
        train(discriminator, classifier, amazon_dataloader, dslr_dataloader, train_epochs[i], domain_label[i],
              loss_weight[i])
