...
def train_generate_samples(discriminator, classifier, dataloader_src, dataloader_tgt, train_epochs, domain_label,
    ...
    迭代100次后开始生成图片
    """
    os.makedirs('../generate_samples/D_A', exist_ok=True)
    ...
    for epoch in range(train_epochs[0]):
        ...
        if epoch >= save_samples_epoch:
            ...
            save_image(generate_imgs, '../generate_samples/D_A/{}.png'.format(epoch), nrow=20, normalize=True)
...
if __name__ == '__main__':
    ...
    # 目标域带标签的
    amazon_dataloader, dslr_dataloader = data_preprocess.get_src_tgt_dataloader2(amazon_path, dslr_path, params.images_name)
    print(len(dslr_dataloader.dataset), len(amazon_dataloader.dataset))
    ...
    train_generate_samples(discriminator, classifier, dslr_dataloader, amazon_dataloader, train_epochs, domain_label,
                           loss_weight, save_samples_epoch)
