...
# @Site    : 
# @File    : transfer_824.py
# @Function: A D C W 相互预测
# @Software: PyCharm
...
def train_3(dataloader, data_name, n):
    # n就是说想以第几个为source domain; n = 0 1 2 3
    for i in range(len(dataloader)):
        if i != n:
            print('source domain:{} -> target domain:{}'.format(data_name[n], data_name[i]))
            train(discriminator, classifier, dataloader[n], dataloader[i], train_epochs, domain_label)
...
if __name__ == '__main__':
    ...
    train_epochs = [100, 20, 20, 20]
    domain_label = [1, 0.0, 1, 0]
    print(len(amazon_dataloader.dataset), len(dslr_dataloader.dataset), len(webcam_dataloader.dataset), len(caltech_dataloader.dataset))

    dataloader = [amazon_dataloader, dslr_dataloader, webcam_dataloader, caltech_dataloader]
    data_name = ['AMAZON', 'DSLR', 'WEBCAM', 'CALTECH']

    train_3(dataloader, data_name, 3)
